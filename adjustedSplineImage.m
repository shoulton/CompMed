...
function [] = adjustedSplineImage()
I =  double(imread('translatedImage.png') > 0);
J =  double(imread('Alz.png') > 0);
[ID, vx, vy] = splineImage(I, J, 20, .01, .001, 1500)

figure;
subplot(2,2,1),imshow(ID), title("Deformed Image");
subplot(2,2,2), imshow(ID - J), title("Deformed Image - Target"), colorbar;
...
for i = 1 : nIter
    ...
    fprintf(file, 'Iter %d of %d, energy is %g\n',i,nIter,E);
